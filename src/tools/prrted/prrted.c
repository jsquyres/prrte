...
int main(int argc, char *argv[])
{
    ...
    if ((int)PRRTE_VPID_INVALID != prrted_debug_failure) {
        prrted_abort=false;
         ...
         */
        if (0 > prrted_debug_failure) {
            prrted_debug_failure = -1*prrted_debug_failure;
            prrted_abort = true;
        }
        /* are we the specified vpid? */
        if ((int)PRRTE_PROC_MY_NAME->vpid == prrted_debug_failure) {
            /* if the user specified we delay, then setup a timer
             * and have it kill us
             */
            if (0 < prrted_debug_failure_delay) {
                PRRTE_TIMER_EVENT(prrted_debug_failure_delay, 0, shutdown_callback, PRRTE_SYS_PRI);

            } else {
                prrte_output(0, "%s is executing clean %s", PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                            prrted_abort ? "abort" : "abnormal termination");
    ...
    /* if we are rank=1, then send our topology back - otherwise, prrte
        ...
        for (i=0; i < argc; i++) {
            if (0 == strcmp("-"PRRTE_MCA_CMD_LINE_ID,  argv[i]) ||
                ...
                if (!ignore) {
                     ...
                     */
                    if (NULL != prrted_cmd_line) {
                        for (j=0; NULL != prrted_cmd_line[j]; j++) {
                            if (0 == strcmp(argv[i+1], prrted_cmd_line[j])) {
                    ...
                    if (!ignore) {
                        prrte_argv_append_nosize(&prrted_cmd_line, argv[i]);
                        prrte_argv_append_nosize(&prrted_cmd_line, argv[i+1]);
                        prrte_argv_append_nosize(&prrted_cmd_line, argv[i+2]);
                    }
                }
                i += 2;
            }
    ...
    if (prrte_debug_daemons_flag) {
        prrte_output(0, "%s prrted: up and running - waiting for commands!", PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
    }
    ret = PRRTE_SUCCESS;
