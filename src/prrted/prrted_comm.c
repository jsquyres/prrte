 ...
 * Globals
 */
static char *get_prrted_comm_cmd_str(int command);
                      ...
                      void* cbdata)
{
    ...
    cmd_str = get_prrted_comm_cmd_str(command);
    PRRTE_OUTPUT_VERBOSE((1, prrte_debug_output,
                         "%s prrted:comm:process_commands() Processing Command: %s",
    ...
    case PRRTE_DAEMON_SIGNAL_LOCAL_PROCS:
        ...
        if (SIGTSTP == signal) {
            if (prrte_debug_daemons_flag) {
                prrte_output(0, "%s prrted_cmd: converted SIGTSTP to SIGSTOP before delivering",
        ...
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received signal_local_procs, delivering signal %d",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                        signal);
        }
    ...
    case PRRTE_DAEMON_PASS_NODE_INFO_CMD:
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received pass_node_info",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
        }
    ...
    case PRRTE_DAEMON_DVM_ADD_PROCS:
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received add_local_procs",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
        }
        ...
        if (PRRTE_SUCCESS != (ret = prrte_odls.launch_local_procs(buffer))) {
            PRRTE_OUTPUT_VERBOSE((1, prrte_debug_output,
                                 "%s prrted:comm:add_procs failed to launch on error %s",
    ...
    case PRRTE_DAEMON_ABORT_PROCS_CALLED:
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received abort_procs report",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
        }
        ...
        for( i = 0; i < num_procs; ++i) {
            ...
            PRRTE_OUTPUT_VERBOSE((2, prrte_debug_output,
                                 "%s prrted:comm:abort_procs Application %s requests term. of %s (%2d of %2d) %3s.",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
        ...
        if( num_new_procs > 0 ) {
            PRRTE_OUTPUT_VERBOSE((2, prrte_debug_output,
                                 "%s prrted:comm:abort_procs Terminating application requested processes (%2d / %2d).",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                 num_new_procs, num_procs));
            prrte_plm.terminate_procs(procs_to_kill);
        } else {
            PRRTE_OUTPUT_VERBOSE((2, prrte_debug_output,
                                 "%s prrted:comm:abort_procs No new application processes to terminating from request (%2d / %2d).",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                 num_new_procs, num_procs));
        }
    ...
    case PRRTE_DAEMON_EXIT_CMD:
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received exit cmd",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
        }
        ...
        /* flag that prrteds were ordered to terminate */
        prrte_prrteds_term_ordered = true;
        ...
        if (0 == (ret = prrte_routed.num_routes())) {
            for (i=0; i < prrte_local_children->size; i++) {
                if (NULL != (proct = (prrte_proc_t*)prrte_pointer_array_get_item(prrte_local_children, i)) &&
                    ...
                    if (prrte_debug_daemons_flag) {
                        prrte_output(0, "%s prrted_cmd: exit cmd, but proc %s is alive",
                                    PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
            ...
            if (prrte_debug_daemons_flag) {
                prrte_output(0, "%s prrted_cmd: all routes and children gone - exiting",
                            PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
            }
            PRRTE_ACTIVATE_JOB_STATE(NULL, PRRTE_JOB_STATE_DAEMONS_TERMINATED);
        } else if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: exit cmd, %d routes still exist",
    ...
    case PRRTE_DAEMON_HALT_VM_CMD:
        if (prrte_debug_daemons_flag) {
            prrte_output(0, "%s prrted_cmd: received halt_vm cmd",
                        PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
        }
