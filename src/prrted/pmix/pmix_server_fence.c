...
static void dmodex_req(int sd, short args, void *cbdata)
{
    ...
    PRRTE_PMIX_CONVERT_PROCT(rc, &prrtenm, &req->tproc);
    if (PRRTE_SUCCESS != rc) {
        PRRTE_ERROR_LOG(rc);
        PRRTE_RELEASE(req);
        return;
    ...
    for (rnum=0; rnum < prrte_pmix_server_globals.reqs.num_rooms; rnum++) {
        ...
        if (r->target.jobid == prrtenm.jobid &&
            r->target.vpid == prrtenm.vpid) {
            /* save the request in the hotel until the
             * data is returned */
            if (PRRTE_SUCCESS != (rc = prrte_hotel_checkin(&prrte_pmix_server_globals.reqs, req, &req->room_num))) {
                prrte_show_help("help-orted.txt", "noroom", true, req->operation, prrte_pmix_server_globals.num_rooms);
    ...
    if (NULL == (jdata = prrte_get_job_data_object(prrtenm.jobid))) {
    ...
    if (PRRTE_VPID_WILDCARD == prrtenm.vpid) {
    ...
    if (NULL == (proct = (prrte_proc_t*)prrte_pointer_array_get_item(jdata->procs, prrtenm.vpid))) {
