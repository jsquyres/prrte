...
int prrte_plm_base_prrted_exit(prrte_daemon_cmd_flag_t command)
{
    int rc;
    prrte_buffer_t *cmd;
...
int prrte_plm_base_prrted_terminate_job(prrte_jobid_t jobid)
{
    ...
    PRRTE_OUTPUT_VERBOSE((5, prrte_plm_base_framework.framework_output,
                         "%s plm:base:prrted_terminate job %s",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
    ...
    if (PRRTE_SUCCESS != (rc = prrte_plm_base_prrted_kill_local_procs(&procs))) {
        PRRTE_ERROR_LOG(rc);
    }
...
int prrte_plm_base_prrted_kill_local_procs(prrte_pointer_array_t *procs)
{
    int rc;
    prrte_buffer_t *cmd;
...
int prrte_plm_base_prrted_signal_local_procs(prrte_jobid_t job, int32_t signal)
{
    int rc;
    prrte_buffer_t cmd;
    ...
    PRRTE_OUTPUT_VERBOSE((5, prrte_plm_base_framework.framework_output,
                         "%s plm:base:prrted_cmd sending signal_local_procs cmds",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
