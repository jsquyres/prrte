...
#include "src/mca/prrteif/base/static-components.h"
...
PRRTE_MCA_BASE_FRAMEWORK_DECLARE(prrte, prrteif, NULL, prrte_if_base_register,
                                 prrte_if_base_open, prrte_if_base_close,
                                 prrte_prrteif_base_static_components, 0);

static int prrte_if_base_register (prrte_mca_base_register_flag_t flags)
{
    prrte_if_do_not_resolve = false;
    (void) prrte_mca_base_framework_var_register (&prrte_prrteif_base_framework, "do_not_resolve",
    ...
    (void) prrte_mca_base_framework_var_register (&prrte_prrteif_base_framework, "retain_loopback",
...
static int prrte_if_base_open (prrte_mca_base_open_flag_t flags)
{
    ...
    return prrte_mca_base_framework_components_open(&prrte_prrteif_base_framework, flags);
}
...
static int prrte_if_base_close(void)
{
    ...
    return prrte_mca_base_framework_components_close(&prrte_prrteif_base_framework, NULL);
}
