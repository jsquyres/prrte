...
static int prrteinstalldirs_env_open(void);


prrte_prrteinstalldirs_base_component_t prrte_prrteinstalldirs_env_component = {
...
#define SET_FIELD(field, envname)                                         \
    do {                                                                  \
        char *tmp = getenv(envname);                                      \
         ...
         prrte_prrteinstalldirs_env_component.install_dirs_data.field = tmp;   \
    } while (0)


static int
prrteinstalldirs_env_open(void)
{
