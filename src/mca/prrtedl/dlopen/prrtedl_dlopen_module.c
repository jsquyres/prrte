                       ...
                       prrte_dl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    if (use_ext && NULL != fname) {
        ...
        for (i = 0, ext = prrte_prrtedl_dlopen_component.filename_suffixes[i];
             NULL != ext;
             ext = prrte_prrtedl_dlopen_component.filename_suffixes[++i]) {
            char *name;
            ...
            /* Yes, the file exists -- try to prrtedlopen it.  If we can't
               prrtedlopen it, bail. */
 ...
 * Module definition
 */
prrte_prrtedl_base_module_t prrte_prrtedl_dlopen_module = {
    .open = dlopen_open,
