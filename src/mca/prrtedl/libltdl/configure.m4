...
AC_DEFUN([MCA_prrte_prrtedl_libltdl_PRIORITY], [50])

#
# Force this component to compile in static-only mode
#
AC_DEFUN([MCA_prrte_prrtedl_libltdl_COMPILE_MODE], [
    AC_MSG_CHECKING([for MCA component $1:$2 compile mode])
    $3="static"
    AC_MSG_RESULT([$$3])
])

# MCA_prrte_prrtedl_libltdl_POST_CONFIG()
# ---------------------------------
AC_DEFUN([MCA_prrte_prrtedl_libltdl_POST_CONFIG],[
    # If we won, then do all the rest of the setup
    AS_IF([test "$1" = "1"],
          [
           # Add some stuff to CPPFLAGS so that the rest of the source
           # tree can be built
           LDFLAGS="$LDFLAGS $prrte_prrtedl_libltdl_ADD_LDFLAGS"
           LIBS="$LIBS $prrte_prrtedl_libltdl_ADD_LIBS"
          ])
])dnl

# MCA_prrtedl_libltdl_CONFIG([action-if-can-compile],
#                       [action-if-cant-compile])
# ------------------------------------------------
AC_DEFUN([MCA_prrte_prrtedl_libltdl_CONFIG],[
    PRRTE_VAR_SCOPE_PUSH([CPPFLAGS_save LDFLAGS_save LIBS_save])
    AC_CONFIG_FILES([src/mca/prrtedl/libltdl/Makefile])
    ...
    prrte_prrtedl_libltdl_happy=no
    AS_IF([test "$with_libltdl" != "no"],
          [AC_MSG_CHECKING([for libltdl dir])
           ...
           PRRTE_CHECK_PACKAGE([prrte_prrtedl_libltdl],
                  [ltprrtedl.h],
                  [ltprrtedl],
                  [lt_prrtedlopen],
                  [],
                  ...
                  [prrte_prrtedl_libltdl_happy=yes],
                  [prrte_prrtedl_libltdl_happy=no])
              ])

    # If we have libltdl, do we have lt_dladvise?
    prrte_prrtedl_libltdl_have_lt_dladvise=0
    AS_IF([test "$prrte_prrtedl_libltdl_happy" = "yes"],
          [CPPFLAGS_save=$CPPFLAGS
           LDFLAGS_save=$LDFLAGS
           LIBS_save=$LIBS

           CPPFLAGS="$prrte_prrtedl_libltdl_CPPFLAGS $CPPFLAGS"
           LDFLAGS="$prrte_prrtedl_libltdl_LDFLAGS $LDFLAGS"
           LIBS="$prrte_prrtedl_libltdl_LIBS $LIBS"
           AC_CHECK_FUNC([lt_dladvise_init],
                         [prrte_prrtedl_libltdl_have_lt_dladvise=1])
           CPPFLAGS=$CPPFLAGS_save
           LDFLAGS=$LDFLAGS_save
           LIBS=$LIBS_save
          ])
    AC_DEFINE_UNQUOTED(PRRTE_DL_LIBLTDL_HAVE_LT_DLADVISE,
        [$prrte_prrtedl_libltdl_have_lt_dladvise],
        [Whether we have lt_dladvise or not])

    AS_IF([test "$prrte_prrtedl_libltdl_happy" = "yes"],
          [prrte_prrtedl_libltdl_ADD_CPPFLAGS=$prrte_prrtedl_libltdl_CPPFLAGS
           prrte_prrtedl_libltdl_ADD_LDFLAGS=$prrte_prrtedl_libltdl_LDFLAGS
           prrte_prrtedl_libltdl_ADD_LIBS=$prrte_prrtedl_libltdl_LIBS
           $1],
    ...
    AC_SUBST(prrte_prrtedl_libltdl_CPPFLAGS)
    AC_SUBST(prrte_prrtedl_libltdl_LDFLAGS)
    AC_SUBST(prrte_prrtedl_libltdl_LIBS)
