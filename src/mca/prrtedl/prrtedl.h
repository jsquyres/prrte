 ...
 * General Description:
 *
 * This framework provides portable access to prrtedlopen- and prrtedlsym-like
 * functionality, very similar to Libtool's libltprrtedl.  Indeed, one of
 * the components in this framework will use libltprrtedl, if it is
 * present/available.  However, on some common types systems where
 * libltprrtedl headers and libraries are *not* available, we can support
 ...
 * installation).  Meaning: the static_components of the prrtedl framework
 * will always contain 0 or 1 components.
 *
 * SIDENOTE: PRRTE used to embed libltprrtedl.  However, as of early
 ...
 * along with a simple component that supports prrtedlopen/prrtedlsym on POSIX
 * platforms and another component that natively uses libltprrtedl seemed
 ...
 *       relevant on this platform (e.g., .so, .prrtedll, .dylib).  If
 ...
 * freed during the call to prrte_prrtedl_base_module_prrtedlclose_fn_t).
 *
 ...
 * change after successive calls to prrte_prrtedl API calls.
 */
typedef int (*prrte_prrtedl_base_module_open_fn_t)
 ...
 * change after successive calls to prrte_prrtedl API calls.
 */
typedef int (*prrte_prrtedl_base_module_lookup_fn_t)
 ...
 */
typedef int (*prrte_prrtedl_base_module_close_fn_t)
 ...
 */
typedef int (*prrte_prrtedl_base_module_foreachfile_fn_t)
 ...
 */
struct prrte_prrtedl_base_component_1_0_0_t {
    /** MCA base component */
...
typedef struct prrte_prrtedl_base_component_1_0_0_t prrte_prrtedl_base_component_1_0_0_t;
typedef struct prrte_prrtedl_base_component_1_0_0_t prrte_prrtedl_base_component_t;

/**
 * Structure for DL modules
 */
struct prrte_prrtedl_base_module_1_0_0_t {
    prrte_mca_base_module_2_0_0_t                 super;

    /** Open / close */
    prrte_prrtedl_base_module_open_fn_t           open;
    prrte_prrtedl_base_module_close_fn_t          close;

    /** Lookup a symbol */
    prrte_prrtedl_base_module_lookup_fn_t         lookup;

    /** Iterate looking for files */
    prrte_prrtedl_base_module_foreachfile_fn_t    foreachfile;
};
typedef struct prrte_prrtedl_base_module_1_0_0_t prrte_prrtedl_base_module_1_0_0_t;
typedef struct prrte_prrtedl_base_module_1_0_0_t prrte_prrtedl_base_module_t;

/**
 * Macro for use in components that are of type DL
 */
#define PRRTE_DL_BASE_VERSION_1_0_0              \
    PRRTE_MCA_BASE_VERSION_2_1_0("prrtedl", 1, 0, 0)
