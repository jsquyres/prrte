...
int prrte_ess_base_prrted_setup(void)
{
    int ret = PRRTE_ERROR;
    int fd;
    ...
    if (prrte_create_session_dirs) {
        ...
        if (prrte_debug_daemons_file_flag) {
            ...
            snprintf(log_file, PATH_MAX, "output-prrted-%s-%s.log",
     ...
     * prrted has no need of the proxy PLM at all
     */
    if (plm_in_use) {
...
int prrte_ess_base_prrted_finalize(void)
{
    prrte_ess_base_signal_t *sig;
    unsigned int i;
