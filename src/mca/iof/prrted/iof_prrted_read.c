...
void prrte_iof_prrted_read_handler(int fd, short event, void *cbdata)
{
    ...
    PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s iof:prrted:read handler read %d bytes from %s, fd %d",
    ...
    if (numbytes <= 0) {
        if (0 > numbytes) {
            ...
            PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                                 "%s iof:prrted:read handler %s Error on connection:%d",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
    ...
    PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s iof:prrted:read handler sending %d bytes to HNP",
