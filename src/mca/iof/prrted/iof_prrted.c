...
static int prrted_push(const prrte_process_name_t* dst_name, prrte_iof_tag_t src_tag, int fd);

static int prrted_pull(const prrte_process_name_t* src_name,
                      prrte_iof_tag_t src_tag,
                      int fd);

static int prrted_close(const prrte_process_name_t* peer,
                       prrte_iof_tag_t source_tag);

static int prrted_output(const prrte_process_name_t* peer,
                        prrte_iof_tag_t source_tag,
                        const char *msg);

static void prrted_complete(const prrte_job_t *jdata);

static int finalize(void);

static int prrted_ft_event(int state);
...
prrte_iof_base_module_t prrte_iof_prrted_module = {
    .init = init,
    .push = prrted_push,
    .pull = prrted_pull,
    .close = prrted_close,
    .output = prrted_output,
    .complete = prrted_complete,
    .finalize = finalize,
    .ft_event = prrted_ft_event
};
...
static int init(void)
{
    ...
    prrte_rml.recv_buffer_nb(PRRTE_NAME_WILDCARD,
                            ...
                            prrte_iof_prrted_recv,
                            NULL);

    /* setup the local global variables */
    PRRTE_CONSTRUCT(&prrte_iof_prrted_component.procs, prrte_list_t);
    prrte_iof_prrted_component.xoff = false;
...
static int prrted_push(const prrte_process_name_t* dst_name,
                      prrte_iof_tag_t src_tag, int fd)
{
   ...
   PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s iof:prrted pushing fd %d for process %s",
    ...
    PRRTE_LIST_FOREACH(proct, &prrte_iof_prrted_component.procs, prrte_iof_proc_t) {
    ...
    prrte_list_append(&prrte_iof_prrted_component.procs, &proct->super);
  ...
  SETUP:
    ...
    if (src_tag & PRRTE_IOF_STDOUT) {
        PRRTE_IOF_READ_EVENT(&proct->revstdout, proct, fd, PRRTE_IOF_STDOUT,
                            prrte_iof_prrted_read_handler, false);
...
static int prrted_pull(const prrte_process_name_t* dst_name,
                      prrte_iof_tag_t src_tag,
                      int fd)
{
    prrte_iof_proc_t *proct;
    ...
    /* this is a local call - only stdin is suppprrted */
    if (PRRTE_IOF_STDIN != src_tag) {
        return PRRTE_ERR_NOT_SUPPORTED;
    }

    PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s iof:prrted pulling fd %d for process %s",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                         fd, PRRTE_NAME_PRINT(dst_name)));
 ...
 * For the prrted, this just means closing the local fd
 */
static int prrted_close(const prrte_process_name_t* peer,
                       prrte_iof_tag_t source_tag)
{
    ...
    PRRTE_LIST_FOREACH(proct, &prrte_iof_prrted_component.procs, prrte_iof_proc_t) {
        if (PRRTE_EQUAL == prrte_util_compare_name_fields(mask, &proct->name, peer)) {
            ...
            if (NULL == proct->stdinev &&
                ...
                prrte_list_remove_item(&prrte_iof_prrted_component.procs, &proct->super);
                PRRTE_RELEASE(proct);
            }
            break;
        }
...
static void prrted_complete(const prrte_job_t *jdata)
{
    ...
    PRRTE_LIST_FOREACH_SAFE(proct, next, &prrte_iof_prrted_component.procs, prrte_iof_proc_t) {
...
static int finalize(void)
{
    ...
    while (NULL != (proct = (prrte_iof_proc_t*)prrte_list_remove_first(&prrte_iof_prrted_component.procs))) {
        if (NULL != proct->revstdout) {
            prrte_iof_base_static_dump_output(proct->revstdout);
        }
        if (NULL != proct->revstderr) {
    ...
    PRRTE_DESTRUCT(&prrte_iof_prrted_component.procs);
...
static void stdin_write_handler(int _fd, short event, void *cbdata)
{
    ...
    PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s prrted:stdin:write:handler writing data to %d",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                         wev->fd));
    ...
    while (NULL != (item = prrte_list_remove_first(&wev->outputs))) {
        ...
        if (0 == output->numbytes) {
            ...
            PRRTE_OUTPUT_VERBOSE((20, prrte_iof_base_framework.framework_output,
                                 "%s iof:prrted closing fd %d on write event due to zero bytes output",
        ...
        PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                             "%s prrted:stdin:write:handler wrote %d bytes",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                             num_written));
        if (num_written < 0) {
            ...
            PRRTE_OUTPUT_VERBOSE((20, prrte_iof_base_framework.framework_output,
                                 "%s iof:prrted closing fd %d on write event due to negative bytes written",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME), wev->fd));
            PRRTE_RELEASE(wev);
            sink->wev = NULL;
            /* tell the HNP to stop sending us stuff */
            if (!prrte_iof_prrted_component.xoff) {
                prrte_iof_prrted_component.xoff = true;
                prrte_iof_prrted_send_xonxoff(PRRTE_IOF_XOFF);
            }
            return;
        } else if (num_written < output->numbytes) {
            PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                                 "%s prrted:stdin:write:handler incomplete write %d - adjusting data",
...
CHECK:
    if (prrte_iof_prrted_component.xoff) {
        ...
        if (prrte_list_get_size(&wev->outputs) < PRRTE_IOF_MAX_INPUT_BUFFERS) {
            /* restart the read */
            prrte_iof_prrted_component.xoff = false;
            prrte_iof_prrted_send_xonxoff(PRRTE_IOF_XON);
        }
    }
}
                        ...
                        const char *msg)
{
    ...
    PRRTE_OUTPUT_VERBOSE((1, prrte_iof_base_framework.framework_output,
                         "%s iof:prrted:output sending %d bytes to HNP",
