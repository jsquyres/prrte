...
static void prrted_abort(int error_code, char *fmt, ...);

/******************
 * prrted module
 ******************/
prrte_errmgr_base_module_t prrte_errmgr_prrted_module = {
    .init = init,
    .finalize = finalize,
...
static void job_errors(int fd, short args, void *cbdata)
{
    ...
    PRRTE_OUTPUT_VERBOSE((1, prrte_errmgr_base_framework.framework_output,
                         "%s errmgr:prrted: job %s reported error state %s",
...
static void proc_errors(int fd, short args, void *cbdata)
{
    ...
    PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                         "%s errmgr:prrted:proc_errors process %s error state %s",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
    ...
    if (prrte_finalizing) {
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted:proc_errors finalizing - ignoring error",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        goto cleanup;
    }
    ...
    if (PRRTE_PROC_STATE_HEARTBEAT_FAILED == state) {
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted:proc_errors heartbeat failed - ignoring error",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        goto cleanup;
    }
    ...
    if (PRRTE_PROC_STATE_LIFELINE_LOST == state ||
        ...
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted lifeline lost or unable to communicate - exiting",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        /* set our exit status */
    ...
    if (NULL == (jdata = prrte_get_job_data_object(proc->jobid))) {
        ...
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted:proc_errors NULL jdata - ignoring error",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        goto cleanup;
    }
    ...
    if (PRRTE_PROC_STATE_COMM_FAILED == state) {
        ...
        if (PRRTE_EQUAL == prrte_util_compare_name_fields(PRRTE_NS_CMP_ALL, PRRTE_PROC_MY_NAME, proc)) {
            PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                                 "%s errmgr:prrted:proc_errors comm_failed to self - ignoring error",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
            goto cleanup;
        }
        /* was it a daemon? */
        if (proc->jobid != PRRTE_PROC_MY_NAME->jobid) {
             ...
             * duplicate code */
            PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                                 "%s errmgr:prrted:proc_errors comm_failed to non-daemon - handling as waitpid",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
            /* get the proc_t */
        ...
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:default:prrted daemon %s exited",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                             PRRTE_NAME_PRINT(proc)));
        ...
        if (prrte_prrteds_term_ordered) {
                ...
                if (NULL != (child = (prrte_proc_t*)prrte_pointer_array_get_item(prrte_local_children, i))) {
                    if (PRRTE_FLAG_TEST(child, PRRTE_PROC_FLAG_ALIVE)) {
                        PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                                             "%s errmgr:default:prrted[%s(%d)] proc %s is alive",
                                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                             __FILE__, __LINE__,
            ...
            if (0 == prrte_routed.num_routes()) {
                PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                                     "%s errmgr:default:prrted all routes gone - exiting",
                                     PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
                PRRTE_ACTIVATE_JOB_STATE(NULL, PRRTE_JOB_STATE_DAEMONS_TERMINATED);
            } else {
                PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                                     "%s errmgr:default:prrted not exiting, num_routes() == %d",
                                     PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
    ...
    if (!PRRTE_FLAG_TEST(child, PRRTE_PROC_FLAG_LOCAL)) {
        PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted:proc_errors proc is not local - ignoring error",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        goto cleanup;
    }

    PRRTE_OUTPUT_VERBOSE((2, prrte_errmgr_base_framework.framework_output,
                         "%s errmgr:prrted got state %s for proc %s",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                         prrte_proc_state_to_str(state),
                         PRRTE_NAME_PRINT(proc)));
    ...
    if (PRRTE_PROC_STATE_TERM_NON_ZERO == state) {
        ...
        if (!prrte_get_attribute(&jdata->attributes, PRRTE_JOB_FAIL_NOTIFIED, NULL, PRRTE_BOOL)) {
            ...
            PRRTE_OUTPUT_VERBOSE((5, prrte_errmgr_base_framework.framework_output,
                                 "%s errmgr:prrted reporting proc %s abnormally terminated with non-zero status (local procs = %d)",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                 PRRTE_NAME_PRINT(&child->name),
    ...
    keep_going:
        ...
        if (!prrte_get_attribute(&jdata->attributes, PRRTE_JOB_FAIL_NOTIFIED, NULL, PRRTE_BOOL)) {
            ...
            PRRTE_OUTPUT_VERBOSE((5, prrte_errmgr_base_framework.framework_output,
                                 "%s errmgr:prrted reporting proc %s aborted to HNP (local procs = %d)",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                 PRRTE_NAME_PRINT(&child->name),
                                 jdata->num_local_procs));
            /* send it */
    ...
    if (!any_live_children(proc->jobid)) {
        ...
        PRRTE_OUTPUT_VERBOSE((5, prrte_errmgr_base_framework.framework_output,
                             "%s errmgr:prrted reporting all procs in %s terminated",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                             PRRTE_JOBID_PRINT(jdata->jobid)));
