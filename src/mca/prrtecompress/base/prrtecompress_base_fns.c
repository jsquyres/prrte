...
int prrte_prrtecompress_base_tar_create(char ** target)
{
    ...
    if( 0 == child_pid ) { /* Child */
        ...
        prrte_output(0, "prrtecompress:base: Tar:: Failed to exec child [%s] status = %d\n", cmd, status);
        exit(PRRTE_ERROR);
    }
    else if(0 < child_pid) {
...
int prrte_prrtecompress_base_tar_extract(char ** target)
{
    int exit_status = PRRTE_SUCCESS;
    char **argv = NULL;
    pid_t child_pid = 0;
