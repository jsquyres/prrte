 ...
 * PRRTED module
 ******************/
prrte_state_base_module_t prrte_state_prrted_module = {
    init,
    finalize,
...
static void track_jobs(int fd, short argc, void *cbdata)
{
    ...
    if (PRRTE_JOB_STATE_LOCAL_LAUNCH_COMPLETE == caddy->job_state) {
        PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                            "%s state:prrted:track_jobs sending local launch complete for job %s",
...
static void track_procs(int fd, short argc, void *cbdata)
{
    ...
    PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                         "%s state:prrted:track_procs called for proc %s state %s",
                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
    ...
    if (PRRTE_PROC_STATE_RUNNING == state) {
        ...
        if (jdata->num_reported == jdata->num_local_procs) {
            ...
            PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                                 "%s state:prrted: notifying HNP all local registered",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        ...
        if (prrte_prrteds_term_ordered &&
            ...
            for (i=0; i < prrte_local_children->size; i++) {
                if (NULL != (pdata = (prrte_proc_t*)prrte_pointer_array_get_item(prrte_local_children, i)) &&
                    ...
                    PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                                         "%s state:prrted all routes gone but proc %s still alive",
                                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
            ...
            PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                                 "%s state:prrted all routes and children gone - exiting",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME)));
        ...
        if (jdata->num_terminated == jdata->num_local_procs &&
            ...
            /* send it */
            PRRTE_OUTPUT_VERBOSE((5, prrte_state_base_framework.framework_output,
                                 "%s state:prrted: SENDING JOB LOCAL TERMINATION UPDATE FOR JOB %s",
                                 PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
            ...
            if (NULL != jdata->map) {
                ...
                for (index = 0; index < map->nodes->size; index++) {
                    ...
                    PRRTE_OUTPUT_VERBOSE((2, prrte_state_base_framework.framework_output,
                                         "%s state:prrted releasing procs from node %s",
                                         PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                                         node->name));
                    for (i = 0; i < node->procs->size; i++) {
                        ...
                        PRRTE_OUTPUT_VERBOSE((2, prrte_state_base_framework.framework_output,
                                             "%s state:prrted releasing proc %s from node %s",
                                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
